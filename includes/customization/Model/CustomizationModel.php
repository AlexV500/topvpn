...
class CustomizationModel extends AbstractModel{
    ...
    {
        $validate = (new Validator)->checkLength($data['customization_name'], 3, 30, 'customization_name', true)
            ->checkLength($data['page_uri'], 2, 10, 'page_uri', true);
    ...
    {
        $validate = (new Validator)->checkLength($data['customization_name'], 3, 30, 'customization_name', true)
            ->checkLength($data['page_uri'], 3, 30, 'page_uri', true);
    ...
    public function deleteRow( array $data) : object{
        ...
        $this->resultMessages->addResultMessage($this->getNameOfClass(), 'ok', $data['customization_name'].'Язык удален успешно');
