...
class LocationAdminList extends AdminList{
    ...
    {
        $output = '';
        $output .= AdminHtmlFormInputs::renderAdminHead('Список Location');
        $output .= '<table id="" class="wp-list-table widefat fixed" cellspacing="0">';
        $output .= AdminHtmlFormInputs::renderAdminHeadOfTableList($this->getColumnDisplayNames(), 'locationlist');
        if ( count($this->getRowsData()) > 0 ) {
            for ( $i = 0; $i < count($this->getRowsData()); $i++ ) {
                ...
                $output .= $result['id'];
                $output .= "</td>";
                $output .= "<td class='topvpnLogo'><img src='". V_CORE_URL .'includes/images/location/'.$result['location_logo']."'></td>";

                $output .= "<td class='topvpnName'>" . stripslashes( wp_filter_nohtml_kses( $result['location_name'] ) ) . $name_suffix . "</td>";
                $output .= "<td class='topvpnSysName'>" . stripslashes( wp_filter_nohtml_kses( $result['location_sys_name'] ) ) . $name_suffix . "</td>";
