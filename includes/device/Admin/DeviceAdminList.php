...
class DeviceAdminList extends AdminList{
    ...
    public function init( array $atts = []) : object
    {
        ...
        $this->setLogoPath(V_PLUGIN_INCLUDES_DIR . 'images/device/');
        $this->unlinkAllUnusedImagesPostHandler('device_logo', $this->getLogoPath());
        $this->setColumnDisplayNames(array(
            'id' => __( 'id', 'topvpn' ),
            'logo' => __('Логотип', 'topvpn'),
            'name'         => __( 'Device', 'topvpn' ),
    ...
    {
        $output = '';
        $output .= AdminHtmlFormInputs::renderAdminHead('Список Device');
        $output .= '<table id="" class="wp-list-table widefat fixed" cellspacing="0">';
        $output .= AdminHtmlFormInputs::renderAdminHeadOfTableList($this->getColumnDisplayNames(), 'devicelist');
        if ( count($this->getRowsData()) > 0 ) {
            for ( $i = 0; $i < count($this->getRowsData()); $i++ ) {
                ...
                $output .= $result['id'];
                $output .= "</td>";
                if (isset($result['device_font_logo_size']) && (trim($result['device_font_logo_size']) !== '')){
                    $size = 'font-size: '.$result['device_font_logo_size'].';';
                } else {
                    $size = 'font-size: 1.4rem;';
                }
                if (isset($result['device_font_logo_color']) && (trim($result['device_font_logo_color']) !== '')){
                    $color = 'color: '.$result['device_font_logo_color'].';';
                } else {
                    $color = 'color: #6c737b;';
                }
                $style = $color .' '. $size;
                if($result['device_font_logo'] == ''){
                    $output .= "<td class='topvpnLogo'><img src='". V_CORE_URL .'includes/images/device/'.$result['device_logo']."' width='21px' height='21px'></td>";
                } else {
                    $output .= '<td class="topvpnLogo"><span class="device-font-logo" style="'.$style.'"><i class="'.$result['device_font_logo'].'"></i></td>';
                }

                $output .= "<td class='topvpnName'>" . stripslashes( wp_filter_nohtml_kses( $result['device_name'] ) ) . $name_suffix . "</td>";
                $output .= "<td class='topvpnSysName'>" . stripslashes( wp_filter_nohtml_kses( $result['device_sys_name'] ) ) . $name_suffix . "</td>";
        ...
        $output .= AdminHtmlFormInputs::renderAdminFormButton('Добавить новый Device', 'Добавить новый Device', 'button button-primary', $this->getCurrentURL(), '&action=add');
        $output .= AdminHtmlFormInputs::renderAdminManageForm($this->countAllRowsFromCustomTable('device_logo'), $this->countFiles($this->getLogoPath()));
        $this->render = $output;
        return $this;
    }
}